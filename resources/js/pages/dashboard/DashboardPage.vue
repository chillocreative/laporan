<template>
    <div>
        <div class="mb-6">
            ...
            <p class="page-subtitle">Selamat kembali, {{ auth.user.value?.name }}</p>
        </div>

        <LoadingSpinner v-if="loading" full-page text="Memuatkan dashboard..." />
        ...
        <template v-else>
            ...
            <template v-if="auth.isSuperAdmin.value">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    ...
                    <StatCard title="Permintaan AI Hari Ini" :value="stats.ai_usage?.total_requests || 0" color="indigo"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="card">
                        <div class="card-header"><h3 class="text-base font-semibold text-gray-900">5 Pelapor Paling Aktif</h3></div>
                        <div class="card-body">
                            <div class="space-y-3">
                                <div v-for="(reporter, index) in stats.top_reporters" :key="reporter.user_id" class="flex items-center justify-between">
                                    ...
                                    <span class="text-sm font-semibold text-gray-900">{{ reporter.report_count }} laporan</span>
                                </div>
                                <div v-if="!stats.top_reporters?.length" class="text-sm text-gray-400 text-center py-4">Tiada laporan lagi</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-header"><h3 class="text-base font-semibold text-gray-900">Laporan Mengikut Tahap Risiko</h3></div>
                        <div class="card-body">
                            <div class="space-y-3">
                                <div v-for="(count, level) in stats.reports?.by_risk_level" :key="level" class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm text-gray-600">{{ riskLabel(level) }}</span>
                                    </div>
            ...
            <template v-else-if="auth.isAdmin.value">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                    <StatCard title="Jumlah Laporan" :value="stats.reports?.total || 0" color="blue"
                        icon="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                    <StatCard title="Jumlah Pengguna" :value="stats.total_users || 0" color="indigo"
                    ...
                    <router-link :to="{ name: 'users.index' }" class="block h-full">
                        <StatCard title="Menunggu Kelulusan" :value="stats.pending_approvals || 0" :color="stats.pending_approvals > 0 ? 'red' : 'green'"
                ...
                <div class="card">
                    <div class="card-header flex items-center justify-between">
                        <h3 class="text-base font-semibold text-gray-900">Laporan Terkini</h3>
                        <router-link :to="{ name: 'reports.index' }" class="text-sm text-primary-600 hover:text-primary-700 font-medium">Lihat Semua</router-link>
                    </div>
                    <div class="card-body p-0">
                        <div v-if="!recentReports.length" class="p-8 text-center text-sm text-gray-400">Tiada laporan lagi.</div>
            ...
            <template v-else>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    <StatCard title="Laporan Saya" :value="stats.reports?.total || 0" color="blue"
                        icon="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                </div>
                ...
                <div class="card">
                    <div class="card-header flex items-center justify-between">
                        <h3 class="text-base font-semibold text-gray-900">Laporan Terkini</h3>
                        <router-link :to="{ name: 'reports.create' }" class="btn-primary btn-sm">Laporan Baru</router-link>
                    </div>
                    <div class="card-body p-0">
                        <div v-if="!recentReports.length" class="p-8 text-center text-sm text-gray-400">Tiada laporan lagi. Cipta laporan pertama anda!</div>
                        <div v-else class="divide-y divide-gray-100">
                            <router-link
                                v-for="report in recentReports"
                                :key="report.id"
...
const statusSummary = computed(() => {
    ...
    return `${pending} menunggu, ${inProgress} aktif`;
});
...
function riskLabel(level) {
    return { low: 'Rendah', medium: 'Sederhana', high: 'Tinggi', critical: 'Kritikal' }[level] || level;
}
