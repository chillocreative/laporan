...
const navItems = computed(() => [
    ...
    { heading: 'Reports', show: true },
    {
        label: 'All Reports', route: 'reports.index', show: true,
        icon: 'M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z',
    },
    {
        label: 'New Report', route: 'reports.create', show: auth.hasPermission('reports.create'),
        icon: 'M12 4.5v15m7.5-7.5h-15',
    },
    { heading: 'Management', show: auth.hasAnyRole(['super-admin', 'admin']) },
    {
        label: 'Users', route: 'users.index', show: auth.hasAnyRole(['super-admin', 'admin']),
        badge: pendingUsersCount.value,
        icon: 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z',
    },
    {
        label: 'Categories', route: 'categories.index', show: auth.hasAnyRole(['super-admin', 'admin']),
    ...
    {
        label: 'Roles', route: 'roles.index', show: auth.isSuperAdmin.value,
        icon: 'M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z',
    },
    { heading: 'System', show: auth.isSuperAdmin.value },
    {
        label: 'Settings', route: 'settings', show: auth.isSuperAdmin.value,
    ...
    {
        label: 'Activity Logs', route: 'logs.activity', show: auth.isSuperAdmin.value,
        icon: 'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z',
    },
    {
        label: 'Security Logs', route: 'logs.security', show: auth.isSuperAdmin.value,
    ...
    {
        label: 'AI Logs', route: 'logs.ai', show: auth.isSuperAdmin.value,
