<template>
    <div class="max-w-3xl mx-auto">
        <h1 class="page-title mb-6">Profil</h1>

        <!-- Profile Info -->
        <div class="card mb-6">
            <div class="card-body">
                <h2 class="text-base font-semibold text-gray-900 mb-4">Maklumat Peribadi</h2>
        ...
        <div class="card">
            <div class="card-body">
                <h2 class="text-base font-semibold text-gray-900 mb-4">Tukar Kata Laluan</h2>
                <form @submit.prevent="handleUpdatePassword" class="space-y-5">
                    ...
                    <div>
                        <label class="label-text">Kata Laluan Semasa *</label>
                    ...
                    <div class="flex justify-end pt-4 border-t border-gray-100">
                        <button type="submit" :disabled="savingPassword" class="btn-primary">
                            {{ savingPassword ? 'Mengemas kini...' : 'Kemas Kini Kata Laluan' }}
                        </button>
                    </div>
                </form>
            </div>
...
async function handleUpdateProfile() {
    ...
    try {
        ...
        notify.success('Profil berjaya dikemas kini!');
    } catch (e) {
        ...
        if (errs) {
            ...
            profileError.value = e.response?.data?.message || 'Gagal mengemas kini profil.';
        }
    } finally {
...
async function handleUpdatePassword() {
    ...
    try {
        ...
        passwordSuccess.value = 'Kata laluan berjaya dikemas kini!';
        notify.success('Kata laluan berjaya dikemas kini!');
    } catch (e) {
        const errs = e.response?.data?.errors;
        if (errs) {
            ...
            passwordError.value = e.response?.data?.message || 'Gagal mengemas kini kata laluan.';
        }
    } finally {
